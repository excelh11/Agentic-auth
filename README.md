...
| 알려진 결함      | `K1~K9` 중 **K4·K6·K7 미해결** | **`K1~K9` 전부 해결** (K6은 오분류로 정정)               |
| 테스트           | 21개                      | **54개**                                                  |
...
에이전트는 이 문서들을 근거로만 판단한다. **명세가 바뀌면 코드보다 문서를 먼저 고친다.**
...
| [`docs/3-TEST.md`](docs/3-TEST.md)   | 테스트 코드 템플릿, 회귀 체크리스트 39항목              |
